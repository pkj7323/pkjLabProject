...
{
	std::int64_t nPerformanceCounter;
	::QueryPerformanceCounter(reinterpret_cast<LARGE_INTEGER*>(&nPerformanceCounter));
...
void CGameTimer::Tick(float fLockFPS)
{
	...
	{
		::QueryPerformanceCounter(reinterpret_cast<LARGE_INTEGER*>(&m_nCurrentTime));
	}
	else
	{
	...
	if (fabsf(fTimeElapsed - m_fTimeElapsed))
	{
...
ULONG CGameTimer::GetFrameRate(LPTSTR lpszString, int nCharacters)
{
	if (lpszString)
	{
		...
		_tcscat_s(lpszString, nCharacters, _T(" FPS)"));
