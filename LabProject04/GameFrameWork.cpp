...
void CGameFramework::CreateDirect3DDevice()
{
...
#endif
	hResult = ::CreateDXGIFactory2(nDXGIFactoryFlags, 
								   __uuidof(IDXGIFactory4), (void**)&m_pdxgiFactory);
	...
	//m_nFenceValues = 0; //Dw 수정 //불안함 뭐지? TODO:
...
void CGameFramework::BuildObjects()
{
	...
	// 1. Scene 객체를 생성하고 BuildObjects를 호출하여 지형과 조명을 만듭니다.
	m_pScene = new CScene();
	if (m_pScene) m_pScene->BuildObjects(m_pd3dDevice, m_pd3dCommandList);

	// 2. 최종 플레이어인 CTankPlayer를 생성합니다. Scene의 지형 정보를 넘겨줍니다.
	m_pPlayer = new CTankPlayer(m_pd3dDevice, m_pd3dCommandList, m_pScene->GetGraphicsRootSignature(), m_pScene->GetTerrain());

	// 3. Scene과 Player가 서로를 알 수 있도록 연결하고, 카메라를 설정합니다.
	if (m_pScene) m_pScene->SetPlayer(m_pPlayer);
	if (m_pPlayer) m_pCamera = m_pPlayer->GetCamera();

	// --- 테스트용 객체 생성 코드 추가 ---
	
	//m_pTestTriangle = new CCubeMeshDiffused(m_pd3dDevice, m_pd3dCommandList, 20.0f, 20.0f, 20.0f);
