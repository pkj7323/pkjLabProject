...
void CTerrainPlayer::OnPlayerUpdateCallback(float fTimeElapsed)
{
	...
	// 목표 Up 벡터 (지형 법선)
	XMFLOAT3 xmf3TargetUp = pTerrain->GetNormal(m_xmf3Position.x, m_xmf3Position.z);

	// 현재 Up 벡터에서 목표 Up 벡터로 부드럽게 보간
	float fUpLerpRatio = 10.0f * fTimeElapsed; // 기울기 반응 속도
	XMFLOAT3 xmf3Up = Vector3::Normalize(Vector3::Lerp(m_xmf3Up, xmf3TargetUp, fUpLerpRatio));

	// 키보드로 회전된 Look 벡터를 새로운 Up 벡터에 맞춰 재조정
	XMFLOAT3 xmf3Look = Vector3::Normalize(Vector3::Subtract(m_xmf3Look, Vector3::ScalarProduct(xmf3Up, Vector3::DotProduct(m_xmf3Look, xmf3Up))));
	...
	: CTerrainPlayer(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, pContext)
{
	...
	XMFLOAT3 xmf3PlayerPos = XMFLOAT3(1000.0f, 0.0f, 1000.0f);
	float fHeight = pTerrain->GetHeight(xmf3PlayerPos.x, xmf3PlayerPos.z) + 5.0f;
	xmf3PlayerPos.y = fHeight;
	SetPosition(xmf3PlayerPos);
	...
	strcpy_s(pBody->m_pstrFrameName, "Body");
	pBody->SetMesh(new CObjMesh(pd3dDevice, pd3dCommandList, "../Assets/M26 Body.obj", 1.0f, false));
	pBody->SetMaterial(0, CreateTankMaterial(XMFLOAT4(0.2f, 0.4f, 0.2f, 1.0f)));
	SetChild(pBody, true);
	Rotate(0, 180, 0); // <-- 이 라인을 삭제합니다.

	// 2. 포탑 (Turret) 로드
	m_pTurret = new CGameObject(1);
	strcpy_s(m_pTurret->m_pstrFrameName, "Turret");
	m_pTurret->SetMesh(new CObjMesh(pd3dDevice, pd3dCommandList, "../Assets/M26 Turret.obj", 1.0f, false));
	m_pTurret->SetMaterial(0, CreateTankMaterial(XMFLOAT4(0.25f, 0.45f, 0.25f, 1.0f)));
	m_pTurret->SetPosition(0.0f, 0.0f, 0.0f);
	pBody->SetChild(m_pTurret, true);
...
void CTankPlayer::Fire(CScene* pScene)
{
	if (pScene) pScene->Fire(this);
}
