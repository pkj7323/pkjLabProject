...
///
CCubeMeshDiffused::CCubeMeshDiffused(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, float fWidth,
									 float fHeight, float fDepth) : CMesh(pd3dDevice, pd3dCommandList)
{
	//직육면체는 꼭지점(정점)이 8개이다.
	m_nVertices = 8;
	m_nStride = sizeof(CDiffusedVertex);
...
CHeightMapImage::CHeightMapImage(LPCTSTR pFileName, int nWidth, int nLength, XMFLOAT3 xmf3Scale)
{
	...
	std::vector<BYTE> tempHeightMap(m_nWidth * m_nLength);

	{
		std::ifstream file(pFileName, std::ios::binary);
		if (!file) throw std::runtime_error("Height map file open failed");

		file.read(reinterpret_cast<char*>(tempHeightMap.data()), tempHeightMap.size());
		if (!file) throw std::runtime_error("Height map file read failed");
	}

	// y축 반전 및 저장 (std::vector 사용)
	m_pHeightMapPixels = new BYTE[m_nWidth * m_nLength];

	for (int y = 0; y < m_nLength; ++y)
	{
		std::copy_n(
			tempHeightMap.begin() + y * m_nWidth,
			m_nWidth,
			m_pHeightMapPixels + (m_nLength - 1 - y) * m_nWidth
		);
	}
}
...
float CHeightMapImage::GetHeight(float fx, float fz)
{
	...
	if (bRightToLeft) //삼각형이 오른쪽에서 왼쪽으로 나열되는 경우
	{
...
CHeightMapGridMesh::CHeightMapGridMesh(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, int xStart,
									   int zStart, int nWidth, int nLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color, void* pContext) : CMesh(pd3dDevice, pd3dCommandList)
{
   ...
   요가 있기 때문에 전체 지형에서 각 격자의 시작 위치를 나타내는 정보가 필요하다.*/
	...
	for (int i = 0, z = zStart; z < (zStart + nLength); z++)
	{
		for (int x = xStart; x < (xStart + nWidth); x++, i++)
		{
			...
			XMFLOAT3 xmf3Position = XMFLOAT3((x * m_xmf3Scale.x), CHeightMapGridMesh::OnGetHeight(x, z, pContext),
											 (z * m_xmf3Scale.z));
			XMFLOAT4 xmf3Color = Vector4::Add(CHeightMapGridMesh::OnGetColor(x, z, pContext), xmf4Color);
	 ...
	 *두 번째 줄의 삼각형들은 오른쪽에서 왼쪽 방향으로나열한다.
	 *즉, 사각형의 줄이 바뀔 때마다 나열 순서가 바뀌도록 한다.
	 *다음 그림의 격자에 대하여 삼각형 스트립을 사용하여 삼각형들을 표현하기 위한 인덱스의 나열은 다음과 같이 격자의 m번째 줄과 (m+1)번째 줄의 정점 번호를
	 *	사각형의 나열 방향에 따라 번갈아 아래, 위, 아래, 위, ... 순서로 나열하면 된다.
