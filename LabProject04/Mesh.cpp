...
CMeshIlluminatedFromFile::CMeshIlluminatedFromFile(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, int nWidth, int nLength, XMFLOAT3 xmf3Scale, void* pContext) : CMeshFromFile(pd3dDevice, pd3dCommandList, new CMeshLoadInfo())
{
	...
	for (int i = 0, z = 0; z < nLength; z++)
	{
		...
		{
			pVertices[i].m_xmf3Position = XMFLOAT3((x * xmf3Scale.x), pHeightMapImage->GetHeight((float)x, (float)z) * xmf3Scale.y, (z * xmf3Scale.z));
...
{
	if (m_pd3dNormalBuffer) // 파일 로드 메쉬 (탱크)
	{
	...
	if (m_nSubMeshes > 0 && nSubSet < m_nSubMeshes) // 파일 로드 메쉬 (탱크)
	{
		pd3dCommandList->IASetIndexBuffer(&(m_pd3dSubSetIndexBufferViews[nSubSet]));
		pd3dCommandList->DrawIndexedInstanced(m_pnSubSetIndices[nSubSet], 1, 0, 0, 0);
	}
	else if (m_nIndices > 0) // 지형 메쉬
	{
		pd3dCommandList->IASetIndexBuffer(&m_d3dIndexBufferView);
		pd3dCommandList->DrawIndexedInstanced(m_nIndices, 1, 0, 0, 0);
	}
...
CHeightMapTerrain::CHeightMapTerrain(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, ID3D12RootSignature* pd3dGraphicsRootSignature, LPCTSTR pFileName, int nWidth, int nLength, int nBlockWidth, int nBlockLength, XMFLOAT3 xmf3Scale) : CGameObject(1)
{
	...
	// 지형은 여러개의 작은 격자(Block)로 나뉘어 처리될 수 있지만, 여기서는 하나의 큰 격자로 처리합니다.
	CMeshIlluminatedFromFile* pMesh = new CMeshIlluminatedFromFile(pd3dDevice, pd3dCommandList, nWidth, nLength, xmf3Scale, m_pHeightMapImage);
	SetMesh(pMesh);
