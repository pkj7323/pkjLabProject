									 ...
									 D3D12_RESOURCE_STATES d3dResourceStates, ID3D12Resource **ppd3dUploadBuffer)
{
	...
	HRESULT hResult = DXCall(pd3dDevice->CreateCommittedResource(&d3dHeapPropertiesDesc,
		...
		switch (d3dHeapType)
		{
			case D3D12_HEAP_TYPE_DEFAULT:
			{
				if (ppd3dUploadBuffer)
				{
					...
					d3dResourceBarrier.Transition.StateBefore = D3D12_RESOURCE_STATE_COPY_DEST; // 수정된 부분
